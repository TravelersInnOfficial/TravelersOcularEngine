...
vec3  Phong (int num) 
{
    ...
    vec3 Ambient = Light[num].Ambient * vec3(texture(myTextureSampler, TexCoords));
  	
    // COMPONENTE DIFUSA 
    vec3 Diffuse = Light[num].Diffuse * max(dot(s, n), 0.0) * vec3(texture(myTextureSampler, TexCoords));  
    
    // COMPONENTE ESPECULAR  
    vec3 Specular = Light[num].Specular * pow(max(dot(r, v), 0.0), 1.0) * 1;
